...
const createConnectedRouter = (structure) => {
		...
		constructor(props, context) {
			...
			this.unsubscribe = context.store.subscribe(() => {
				...
				const {
					...
					hash: hashInStore,
				} = context.store.getState().toJS().router.location;
				...
				if (pathnameInHistory !== pathnameInStore || searchInHistory !==
					searchInStore || hashInHistory !== hashInStore) {
	...
	const mapDispatchToProps = dispatch => ({
		onLocationChanged: (location, action) => dispatch(
